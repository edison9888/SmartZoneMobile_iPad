...
const static CGFloat kReflectionFraction = 0.55;
...
- (void)setDefaultImage:(UIImage *)newDefaultImage {
	...
	defaultImage = newDefaultImage;//[[newDefaultImage addImageReflection:kReflectionFraction] retain];
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	startPosition = (startPoint.x / 1.5) + scrollView.contentOffset.x;
		//NSLog(@"startPosition ===== %f" , startPosition);
	if (isSingleTap)
		isDoubleTap = YES;
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	if (newCover != selectedCoverView.number) {
		if (newCover < 0)
			...
			if ([self.viewDelegate respondsToSelector:@selector(openFlowView:selectionDidMoveOn:)])
				[self.viewDelegate openFlowView:self selectionDidMoveOn:newCover];
		}
	}
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	if (isSingleTap) {
		...
		AFItemView *targetCover = [self findCoverOnscreen:targetLayer];
			...
			//이미지 선택을 위해 기존소스 변경
		targetCover = [self findCoverOnscreen:targetLayer];
		if (targetCover && (targetCover.number != selectedCoverView.number)){
			[self setSelectedCover:targetCover.number];
		}else if (targetCover) {
				//NSLog(@"select image%d", targetCover.number);
				// send menu select noti !!
			[self.viewDelegate openFlowView:self imageSelected:targetCover.number];
	...
	[self centerOnSelectedCover:YES];
		...
		//커버뷰 선택하고 다른 커버뷰를 갔다가 오면 선택됬던 커버뷰 이벤트를 못받아서 수정 
	if (beginningCover != selectedCoverView.number || selectimage.number != selectedCoverView.number)
