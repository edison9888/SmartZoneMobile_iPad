...
- (void)viewWillDisappear:(BOOL)animated
{
	...
	if ([UIApplication sharedApplication].statusBarOrientation == UIInterfaceOrientationLandscapeLeft
		...
		NSArray *newArray = [NSArray arrayWithObjects:[[toolbar items] objectAtIndex:0], [[toolbar items] objectAtIndex:1], nil];
...
-(void)didEndCommunication:(NSDictionary *)_resultDic requestDictionary:(NSMutableDictionary *)_dic{
	...
	if (resultNum == 0 && rslt!=nil) {
		...
		if ([[self.contentDictionary objectForKey:@"delyn"] isEqualToString:@"Y"]) {
			...
			NSMutableArray *items = [[toolbar items] mutableCopy];
			if([items lastObject] == deleteBarButton){
				// do nothing
				//NSLog(@"%@, %@", [items objectAtIndex:0], barButtonItem);
			} else {
		...
		[tempView loadHTMLString:[self.contentDictionary objectForKey:@"content"] baseURL:nil];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
	//UITextView *tempView;
	CGSize newSize = [[self.contentDictionary objectForKey:@"bulltitle"]
...
{	
	...
	}else if (alertView.tag == 7979) { // reload current page & list
		//[self loadDetailContentAtIndex:bullid];
		[self.contentTableView reloadData];
		[listView loadData];
		
	}else if (alertView.tag == 7980) { // reload current page
		//[self loadDetailContentAtIndex:bullid];
		[self.contentTableView reloadData];
